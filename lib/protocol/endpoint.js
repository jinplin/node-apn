...
function setupEventsForSocket(protocol, socket) {
...
module.exports = function(dependencies) {
  ...
  function Endpoint(options) {
    ...
    setupEventsForSocket = setupEventsForSocket.bind(this);
  ...
  Endpoint.prototype._connect = function connect(completeSetup) {
    if (this.options.proxyHost && this.options.proxyPort) {
      var agent = new HttpsProxyAgent({
        proxyHost: this.options.proxyHost,
        proxyPort: this.options.proxyPort
      });
      agent.createConnection(this.options, function(_, cts, socket) {
        this._socket = cts;
        setupEventsForSocket(protocol, socket);
        completeSetup();
        this._connected();
      }.bind(this));
    } else {
      this._socket = tls.connect(this.options);
      setupEventsForSocket(protocol, this._socket);
      completeSetup();
    }
  };
